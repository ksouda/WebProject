...
final class PromotionController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        $promotion->setStartDate(new \DateTime()); // Automatically set the start date

        $form = $this->createForm(PromotionType::class, $promotion);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            // Update the price of the product(s) associated with the promotion
            foreach ($promotion->getIdProduit() as $produit) {
                $produit->setPrix($promotion->getPrixNouv());
